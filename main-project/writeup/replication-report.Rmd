...
probs = d %>% filter(qnum %in% c(0, 2, 3, 4, 5)) %>%
...
```{r echo=T}
cond.probs = probs %>% group_by(question, story, workerid) %>%
  summarise(probability = mean(probability))

answer1 = cond.probs %>% filter(question == 'baseline') %>% select(workerid, story, probability)
answer2b = cond.probs %>% filter(question == 'counterexample') %>% select(workerid, story, probability)
answer3 = cond.probs %>%  filter(question == 'generalization') %>% select(workerid, story, probability)
...
answers = data.frame(workerid = answer1$workerid,
                     story = answer1$story,
                     answer1 = num(answer1$probability),
                     answer2b = num(answer2b$probability),
                     answer3 = num(answer3$probability))
...
We ran a paired t-test on the comparison between a participant's average responses to question 1 (baseline) and question 2b (counterexample). In a pilot with `r replication.n` naive participants, we do not find evidence that people revise their causal beliefs about other aspects of a particular situation (t(`r comp1$parameter`)=`r comp1$statistic`, p=`r comp1$p.value`).
...
Because the specific scenarios might matter to participants' responses, we will also run a mixed effects linear regression with random effects for scenario.

```{r}
fit = probs %>% filter(question %in% c('baseline', 'counterexample')) %>%
  lmer(probability ~ question + (1 | story), .)
summary(fit)
```
...
```{r}
lm.fit = lm(probability ~ question, probs)
lmer.fit = lmer(probability ~ question + (1+question|story) + (1+question|workerid), probs)
```
