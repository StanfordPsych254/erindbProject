...
ungrammatical = d[d$qtype=='ungrammatical' & d$response==T & !is.na(d$response),] %>%
  select(qnum, story)
ungrammatical.explanations = d$response[d$qtype=='explanation' & d$story%in%ungrammatical$story]
```
...
for (i in 1:nrow(ungrammatical)) {
  ...
  d = d %>% filter(!(qnum==exclude.qnum & story==exclude.story))
}
```
...
comp1 = with(cond.probs, t.test(baseline, counterexample))
comp2 = with(cond.probs, t.test(baseline, generalization))
replication.n = length(unique(d$workerid))
```
    
The main comparison that I will consider a replication is Comparison 1.

We ran a paired t-test on the comparison between a participant's average responses to question 1 (baseline) and question 2b (counterexample). We (do not) find evidence that people revise their causal beliefs about other aspects of a particular situation (t(`r round(comp1$parameter, digits=2)`)=`r round(comp1$statistic, digits=2)`, p=`r round(comp1$p.value, digits=2)`).
...
We again ran a paired t-test and (did not) find evidence that these participants generalize this revision to other situations (t(`r round(comp2$parameter, digits=2)`)=`r round(comp2$statistic, digits=2)`, p=`r round(comp2$p.value, digits=2)`).
...
We (do not) find a significant effect of question for comparison 1, b=`r round(coefs1$Estimate[2], digits=2)`, t(`r round(coefs1$df[2], digits=2)`)=`r round(coefs1$t.value[2], digits=2)`, p=`r round(coefs1$Pr...t..[2], digits=2)`.

We (do not) find a significant effect of question for comparison 2, b=`r round(coefs2$Estimate[2], digits=2)`, t(`r round(coefs2$df[2], digits=2)`)=`r round(coefs2$t.value[2], digits=2)`, p=`r round(coefs2$Pr...t..[2], digits=2)`.
