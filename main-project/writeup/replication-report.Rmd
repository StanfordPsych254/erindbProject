...
In a pilot with `r replication.n` non-naive participants, we do not find evidence that people revise their causal beliefs about other aspects of a particular situation (t(`r comp1$parameter`)=`r comp1$statistic`, p=`r comp1$p.value`), nor that they generalize this revision to other situations (t(`r comp2$parameter`)=`r comp2$statistic`, p=`r comp2$p.value`).

```{r, fig.width=5, fig.height=3}
d %>% filter(qnum %in% c(0, 2, 3)) %>%
  mutate(question = factor(as.character(qnum),
                           levels = c('0', '2', '3'),
                           labels = c('question1', 'question2b', 'question3')),
         probability = num(response)) %>%
  group_by(question) %>%
  summarise(mean.rating = mean(probability),
            cil = ci.low(probability), 
            cih = ci.high(probability))  %>%
  ggplot(., aes(x=question, y=mean.rating)) +
